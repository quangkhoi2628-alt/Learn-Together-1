...
export const getQuickAnswer = async (grade: number, subject: string, problem: string): Promise<string> => {
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: `Hãy giải nhanh và đưa ra đáp án trực tiếp cho bài tập sau đây dành cho học sinh lớp ${grade} môn ${subject}: "${problem}". Chỉ cần đưa ra đáp án cuối cùng một cách ngắn gọn. Sử dụng LaTeX cho các công thức toán học (ví dụ: $x=5$). KHÔNG BAO GIỜ thoát (escape) các dấu đô la.`,
      config: {
...
export const generateFlashcards = async (text: string): Promise<Flashcard[] | null> => {
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: `Tạo flashcard từ đoạn văn bản sau. Mỗi flashcard cần có một câu hỏi rõ ràng và câu trả lời ngắn gọn. Nếu có công thức toán học hoặc hóa học, hãy sử dụng cú pháp LaTeX (ví dụ: $x^2 + y^2 = z^2$). KHÔNG BAO GIỜ thoát (escape) các dấu đô la. Văn bản: "${text}"`,
      config: {
...
- Nếu có công thức toán học/hóa học, hãy sử dụng cú pháp LaTeX (ví dụ: $x^2=4$).
...
export const generatePracticeQuestionsFromPdf = async (base64Pdf: string, numQuestions: number): Promise<{ subject: Subject, questions: PracticeQuestion[] } | null> => {
  try {
    ...
    const textPart = {
      ...
      Mỗi câu hỏi phải có:
      1.  Một câu hỏi rõ ràng ('question'). Đối với câu hỏi toán học, sử dụng cú pháp LaTeX. KHÔNG BAO GIỜ thoát (escape) các dấu đô la (ví dụ: $x=1$, không phải \\$x=1\\$).
...
export const generatePracticeQuestionsFromTopic = async (subject: Subject, topic: string, numQuestions: number): Promise<PracticeQuestion[] | null> => {
  try {
      const response = await ai.models.generateContent({
          ...
          Mỗi câu hỏi phải có:
          1. Một câu hỏi rõ ràng ('question'). Đối với câu hỏi toán học, BẮT BUỘC sử dụng cú pháp LaTeX. TUYỆT ĐỐI KHÔNG escape (thoát) dấu đô la (ví dụ: $x=1$, không phải \\$x=1\\$).
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      **Câu hỏi của học sinh:** "${userQuestion}"
      ...
      Hãy giải đáp thắc mắc của học sinh một cách ngắn gọn, rõ ràng và tập trung vào câu hỏi. Sử dụng cú pháp LaTeX cho các công thức toán học, không bao giờ escape dấu đô la.`,
      config: {
...
export const generateFollowUpExercises = async (subject: Subject, topic: string, weaknesses: string[]): Promise<PracticeQuestion[] | null> => {
  try {
    const response = await ai.models.generateContent({
      ...
      Hãy tạo ra 2 câu hỏi trắc nghiệm mới để giúp học sinh luyện tập và khắc phục những điểm yếu này. Các câu hỏi phải liên quan trực tiếp đến các điểm yếu đã nêu. Sử dụng cú pháp LaTeX cho các công thức toán học. KHÔNG BAO GIỜ thoát (escape) các dấu đô la.`,
      config: {
        responseMimeType: "application/json",
        responseSchema: {
            type: Type.ARRAY,
...
export const generateMixedPracticeTest = async (subject: Subject, topic: string, numMcq: number, numOpenEnded: number): Promise<{ mcq: PracticeQuestion[], openEnded: OpenEndedQuestion[] } | null> => {
  ...
  const prompt = isEnglish
    ...
    : `Bạn là một AI chuyên tạo đề kiểm tra cho học sinh lớp 9.
      ...
      Với mỗi câu hỏi trắc nghiệm, hãy cung cấp: 'question' (câu hỏi), bốn 'options' (lựa chọn), 'correctAnswer' (đáp án đúng), và trường 'topic' với giá trị là "${topic}". Sử dụng cú pháp LaTeX cho các công thức toán học. KHÔNG BAO GIỜ thoát (escape) các dấu đô la.
      Với mỗi câu hỏi tự luận, hãy cung cấp: 'question' (câu hỏi), 'suggestedAnswer' (gợi ý trả lời), và trường 'topic' với giá trị là "${topic}". Sử dụng cú pháp LaTeX cho các công thức toán học. KHÔNG BAO GIỜ thoát (escape) các dấu đô la.
